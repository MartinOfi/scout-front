/**
 * Personas Externas Detail Component
...
@Component({
  selector: 'app-personas-externas-detail',
  standalone: true,
  ...
  templateUrl: './personas-externas-detail.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PersonasExternasDetailComponent implements OnInit {
  ...
  readonly personaExterna: Signal<PersonaExterna | null> = computed(() => this.state.selected());
  ...
  ngOnInit(): void {
    this.route.params.pipe(take(1)).subscribe(params => {
      ...
      if (id) {
        ...
        if (this.state.personasExternas().length === 0) this.state.load();
      }
    });
  }

  onEdit(): void {
    const id = this.personaExterna()?.id;
    if (id) this.router.navigate(['/personas/personas-externas', id, 'editar']);
  }
  ...
  onDelete(): void {
    ...
    this.confirmDialog.confirmDelete('persona externa').subscribe((confirmed: boolean) => {
      if (confirmed) {
        this.state.delete(id).subscribe({ next: () => this.router.navigate(['/personas/personas-externas']) });
      }
    });
  }
